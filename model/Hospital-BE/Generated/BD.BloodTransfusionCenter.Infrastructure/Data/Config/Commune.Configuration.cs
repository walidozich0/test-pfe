...
namespace BD.BloodTransfusionCenter.Core
{
    ...
    public partial class CommuneConfiguration : IEntityTypeConfiguration<Commune>
    {
        ...
        public void Configure(EntityTypeBuilder<Commune> builder)
        {
            ...
            builder.HasKey(@"Id");
            builder.HasOne(x => x.Wilaya).WithMany(op => op.Communes).OnDelete(DeleteBehavior.Restrict).HasForeignKey(@"WilayaId").IsRequired(true);
            builder.HasMany(x => x.Donors).WithOne(op => op.Commune).OnDelete(DeleteBehavior.Restrict).HasForeignKey(@"CommuneId").IsRequired(true);
