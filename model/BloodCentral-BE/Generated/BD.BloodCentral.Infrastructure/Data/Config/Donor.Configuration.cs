...
namespace BD.BloodCentral.Core
{
    ...
    public partial class DonorConfiguration : IEntityTypeConfiguration<Donor>
    {
        ...
        public void Configure(EntityTypeBuilder<Donor> builder)
        {
            ...
            builder.Property(x => x.WantToStayAnonymous).HasColumnName(@"WantToStayAnonymous").IsRequired().ValueGeneratedNever();
            builder.Property(x => x.ExcludeFromPublicPortal).HasColumnName(@"ExcludeFromPublicPortal").IsRequired().ValueGeneratedNever();
            builder.Property(x => x.Availability).HasColumnName(@"Availability").ValueGeneratedNever();
            builder.Property(x => x.ContactMethod).HasColumnName(@"ContactMethod").ValueGeneratedNever();
            ...
            builder.Property(x => x.CommuneId1).HasColumnName(@"CommuneId1").ValueGeneratedNever();
            builder.HasKey(@"Id");
            builder.HasOne(x => x.Commune).WithMany(op => op.Donors).OnDelete(DeleteBehavior.Restrict).HasForeignKey(@"CommuneId1").IsRequired(true);
            builder.HasMany(x => x.DonorBloodTransferCenterSubscriptions).WithOne(op => op.Donor).OnDelete(DeleteBehavior.Restrict).HasForeignKey(@"DonorId").IsRequired(true);
            builder.HasMany(x => x.BloodDonationPledges).WithOne(op => op.Donor).HasForeignKey(@"DonorId").IsRequired(true);
